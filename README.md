[![MseeP.ai Security Assessment Badge](https://mseep.net/pr/devbrother2024-my-mcp-server-251209-badge.png)](https://mseep.ai/app/devbrother2024-my-mcp-server-251209)
...
├── src/
│   └── index.ts          # MCP 서버 메인 진입점 (createServer export)
├── .smithery/            # Smithery 빌드 출력 디렉토리 (빌드 후 생성)
...
```bash
# Smithery 개발 서버 실행 (인터랙티브 플레이그라운드)
npm run dev
```

빌드가 성공하면 `.smithery/` 디렉토리에 번들된 파일이 생성됩니다.
...
-   **@smithery/cli**: Smithery 빌드 및 배포 CLI (devDependencies)

## 🔧 스크립트

-   `npm run build`: Smithery 빌드 (배포용)
-   `npm run dev`: Smithery 개발 서버 실행 (인터랙티브 플레이그라운드)
-   `npm run build:local`: TypeScript를 JavaScript로 컴파일 (로컬 개발용)

## 🚀 Smithery 배포

### 1. GitHub에 코드 푸시

프로젝트를 GitHub 저장소에 푸시합니다 (`smithery.yaml` 포함).

### 2. Smithery에 연결

1. [Smithery](https://smithery.ai)에서 GitHub 저장소를 연결합니다.
2. 서버 페이지에서 Deployments 탭으로 이동합니다.
3. "Deploy" 버튼을 클릭하여 빌드 및 배포합니다.

### 배포 설정

`smithery.yaml` 파일에서 배포 옵션을 설정할 수 있습니다:
...
-   [Smithery TypeScript 배포 문서](https://smithery.ai/docs/build/deployments/typescript)
